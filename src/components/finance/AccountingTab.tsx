...
import { Download, Printer, BookOpen, Receipt, ShoppingCart, Users, Wallet, CalendarClock, Info, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <table className="w-full text-[11px]">
      <thead>
        <tr className="text-muted-foreground bg-muted/30">
          {headers.map((h, i) => <th key={i} className={cn('px-3 py-1.5 font-semibold whitespace-nowrap', (align?.[i] || (i === 0 ? 'l' : 'r')) === 'r' ? 'text-right' : 'text-left')}>{h}</th>)}
        </tr>
      </thead>
      <tbody>
        ...
        {rows.map((r, i) => (
          <tr key={i} className="border-t border-border">
            {r.map((c, j) => <td key={j} className={cn('px-3 py-1.5 whitespace-nowrap', (align?.[j] || (j === 0 ? 'l' : 'r')) === 'r' ? 'text-right' : 'text-left')}>{c}</td>)}
          </tr>
        ))}
      </tbody>
      {footer && (
        <tfoot>
          <tr className="border-t-2 border-brand-primary/30 font-bold text-brand-dark bg-brand-card">
            {footer.map((c, j) => <td key={j} className={cn('px-3 py-2 whitespace-nowrap', (align?.[j] || (j === 0 ? 'l' : 'r')) === 'r' ? 'text-right' : 'text-left')}>{c}</td>)}
          </tr>
        </tfoot>
      )}
    </table>
        ...
        <>
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            ...
            <Section title="Ventas por medio de pago" icon={Wallet}>
              <Table headers={['Medio', 'Comprobantes', 'Total', '% del total']} rows={data.sales.byPayment.map((p: any) => [p.label, p.count, formatPrice(p.total), `${data.sales.totals.gross ? Math.round((p.total / data.sales.totals.gross) * 100) : 0}%`])} footer={['Total', data.sales.totals.count, formatPrice(data.sales.totals.gross), '100%']} />
            </Section>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            ...
            <Section title="Cierres de caja (arqueos)" icon={Wallet}>
              <Table headers={['Cierre', 'Cajero', 'Ventas', 'Esperado', 'Contado', 'Diferencia']} align={['l', 'l', 'r', 'r', 'r', 'r']}
