    ...
    <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-5 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-2xl bg-[#364266] text-[#FEF3DE] font-semibold text-xs hover:bg-[#242D49] shadow-md transition-all self-start sm:self-auto font-sans"
        >
          <Plus size={15} />
      ...
      <div className="bg-white rounded-2xl p-3 border border-[#364266]/10 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 font-sans">
        {/* Search */}
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
          ...
          <div className="flex bg-gray-100 p-0.5 rounded-xl border border-gray-200 ml-2">
            <button
              ...
              className={cn('p-1.5 rounded-lg text-xs', viewMode === 'cards' ? 'bg-white text-[#364266] shadow-sm' : 'text-gray-500')}
            ...
            <button
              ...
              className={cn('p-1.5 rounded-lg text-xs', viewMode === 'list' ? 'bg-white text-[#364266] shadow-sm' : 'text-gray-500')}
              title="Vista Lista"
            >
              <List size={14} />
            </button>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar font-sans">
        {statusTabs.map(t => (
          <button
            ...
            className={cn(
              ...
              activeTab === t.status
                ? 'bg-[#FAF8EA] text-[#242D49] border border-[#C6BF81] shadow-sm'
        ...
        <span className="text-xs text-gray-500 ml-auto font-semibold font-sans">
          {filtered.length} pedidos • Total: <strong className="text-[#242D49]">{formatPrice(summarySales)}</strong>
        </span>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 font-sans">
          {filtered.length === 0 ? (
            ...
            filtered.map(order => (
              <div
                key={order.id}
                className="bg-white rounded-2xl p-4 border border-[#364266]/10 shadow-sm hover:border-[#C6BF81] transition-all flex flex-col justify-between"
              >
                <div>
                  {/* Card Header */}
                  <div className="flex items-start justify-between gap-2 pb-2 border-b border-gray-100">
                    <div>
                      <span className="font-bold text-sm text-[#364266] font-sans">{orderNumber(order.id)}</span>
                  ...
                  {/* Customer */}
                  <div className="py-2 text-xs text-[#242D49] font-sans">
                  ...
                  <div className="py-2 border-t border-dashed border-gray-200 space-y-1 text-xs font-sans">
                    {order.items.map((it, idx) => (
                      <div key={idx} className="flex justify-between">
                        ...
                        <span className="font-bold text-[#364266]">{formatPrice(it.price * it.quantity)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="pt-3 border-t border-gray-100 flex items-center justify-between mt-2">
                  <div>
                    ...
                    <span className="font-bold text-base text-[#242D49] font-sans">{formatPrice(order.total)}</span>
                  </div>

                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => setSelectedInvoice(order)}
                      className="p-2 rounded-xl bg-gray-100 hover:bg-[#FAF8EA] text-[#364266] transition-colors"
                      title="Ver Factura"
                    >
                      <Eye size={15} />
                    </button>
        ...
        <div className="bg-white rounded-2xl border border-[#364266]/10 shadow-sm overflow-hidden font-sans">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#FAF8EA] text-[#364266] font-bold border-b border-[#364266]/10">
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map(order => (
                  <tr key={order.id} className="hover:bg-[#FAF8EA]/50 transition-colors">
                    <td className="py-3 px-4 font-bold text-[#364266] font-sans">{orderNumber(order.id)}</td>
                    <td className="py-3 px-4 text-gray-500 font-sans">{order.createdAt}</td>
                    <td className="py-3 px-4 font-semibold text-[#242D49] font-sans">{order.customer.name}</td>
                    <td className="py-3 px-4 text-gray-600 truncate max-w-xs font-sans">
                      {order.items.map(i => `${i.quantity}x ${i.name}`).join(', ')}
                    </td>
                    <td className="py-3 px-4 font-sans">
                      <span className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-[#FAF8EA] text-[#364266] border border-[#C6BF81]/40">
                        {order.paymentMethod === 'cash' ? 'Efectivo' :
                         order.paymentMethod === 'card_debit' ? 'Tarjeta Débito' :
                         order.paymentMethod === 'card_credit' ? 'Tarjeta Crédito' : 'QR Transferencia'}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-right font-bold text-[#242D49] font-sans">{formatPrice(order.total)}</td>
                    <td className="py-3 px-4 font-sans">
                    ...
                    <td className="py-3 px-4 text-center font-sans">
                      <div className="flex items-center justify-center gap-1.5">
                        <button
                          onClick={() => setSelectedInvoice(order)}
                          className="p-1.5 rounded-lg hover:bg-gray-100 text-[#364266]"
                          title="Ver Factura"
                        >
                          <Eye size={15} />
                        </button>
